...
import qs.Services
import qs.Modules.Linux.RightSidebar.Calendar
...
Rectangle {
    ...
    property var tabs: [
        {
         ...
         {
            "type": "todo",
            "name": TranslationService.tr("To Do"),
            "icon": "done_outline",
            "widget": "todo/TodoWidget.qml"
        },
        {
            "type": "timer",
            "name": TranslationService.tr("Timer"),
            "icon": "schedule",
    ...
    RowLayout {
        ...
        StyledText {
            property int remainingTasks: TodoService.list.filter(task => !task.done).length
            Layout.margins: 10
            Layout.leftMargin: 0
            text: TranslationService.tr("%1   •   %2 tasks").arg(DateTimeService.collapsedCalendarFormat).arg(remainingTasks)
