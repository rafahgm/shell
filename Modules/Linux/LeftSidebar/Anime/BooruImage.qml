                ...
                Rectangle {
                    ...
                    ColumnLayout {
                        ...
                        MenuButton {
                            ...
                            buttonText: TranslationService.tr("Open file link")
                            onClicked: {
                        ...
                        MenuButton {
                            ...
                            Layout.fillWidth: true
                            buttonText: TranslationService.tr("Go to source (%1)").arg(StringUtils.getDomain(root.imageData.source))
                        ...
                        MenuButton {
                            id: downloadButton
                            Layout.fillWidth: true
                            buttonText: TranslationService.tr("Download")
                            onClicked: {
                                root.showActions = false;
                                const targetPath = root.imageData.is_nsfw ? root.nsfwPath : root.downloadPath;
                                Quickshell.execDetached(["bash", "-c", 
                                    `mkdir -p '${targetPath}' && curl '${root.imageData.file_url}' -o '${targetPath}/${root.fileName}' && notify-send '${TranslationService.tr("Download complete")}' '${root.downloadPath}/${root.fileName}' -a 'Shell'`
                                ])
                            }
                        }
                    }
