...
MouseArea { // Notification group area
    ...
    onContainsMouseChanged: {
        ...
        if (root.containsMouse) {
            root.notifications.forEach(notif => {
                NotificationsService.cancelTimeout(notif.notificationId);
            });
        } else {
            root.notifications.forEach(notif => {
                NotificationsService.restartTimeout(notif.notificationId);
            });
        }
    }
    ...
    SequentialAnimation { // Drag finish animation
        ...
        onFinished: () => {
            root.notifications.forEach(notif => {
                Qt.callLater(() => {
                    NotificationsService.discardNotification(notif.notificationId);
                });
            });
        }
    }
            ...
            ColumnLayout {
                ...
                Item { // App name (or summary when there's only 1 notif) and time
                    ...
                    NotificationGroupExpandButton {
                        ...
                        StyledToolTip {
                            text: TranslationService.tr("Tip: right-clicking a group\nalso expands it")
                        }
                    }
                }
