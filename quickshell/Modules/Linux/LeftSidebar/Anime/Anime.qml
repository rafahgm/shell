...
Item {
    ...
    readonly property var responses: BooruService.responses
    ...
    Connections {
        ...
        function onRunningRequestsChanged() {
            if (BooruService.runningRequests === 0) {
    ...
    property var allCommands: [
        {
            name: "mode",
            description: TranslationService.tr("Set the current API provider"),
            execute: (args) => {
                BooruService.setProvider(args[0]);
            }
        },
        {
            name: "clear",
            description: TranslationService.tr("Clear the current list of images"),
            execute: () => {
        ...
        {
            name: "next",
            description: TranslationService.tr("Get the next page of results"),
            execute: () => {
        ...
        {
            name: "safe",
            description: TranslationService.tr("Disable NSFW content"),
            execute: () => {
        ...
        {
            name: "lewd",
            description: TranslationService.tr("Allow NSFW content"),
            execute: () => {
    ...
    function handleInput(inputText) {
        if (inputText.startsWith(root.commandPrefix)) {
            ...
            if (commandObj) {
                ...
                BooruService.addSystemMessage(TranslationService.tr("Unknown command: ") + command);
            }
        }
            ...
            BooruService.makeRequest(tagList, Persistent.states.booru.allowNsfw, Config.options.sidebar.booru.limit, pageIndex);
        }
    }
    ...
    ColumnLayout {
        ...
        Item {
            ...
            StyledListView { // BooruService responses
            ...
            PagePlaceholder {
                ...
                icon: "bookmark_heart"
                title: TranslationService.tr("Anime boorus")
                description: ""
            ...
            MaterialLoadingIndicator {
                ...
                loading: root.pullLoading || BooruService.runningRequests > 0
        ...
        Rectangle { // Tag input area
            ...
            RowLayout { // Input field and send button
                ...
                StyledTextArea { // The actual TextArea
                    ...
                    placeholderText: TranslationService.tr('Enter tags, or "%1" for commands').arg(root.commandPrefix)
                    ...
                    property Timer searchTimer: Timer { // Timer for tag suggestions
                        ...
                        onTriggered: {
                            ...
                            if (words.length > 0) {
                                BooruService.triggerTagSearch(words[words.length - 1]);
                            }
                        }
                    }
                    ...
                    onTextChanged: { // Handle tag suggestions
                        ...
                        if(tagInputField.text.startsWith(`${root.commandPrefix}mode`)) {
                            ...
                            const providerResults = Fuzzy.go(root.suggestionQuery, BooruService.providerList.map(provider => {
                                return {
                            ...
                            root.suggestionList = providerResults.map(provider => {
                                return {
                                    ...
                                    displayName: `${BooruService.providers[provider.target].name}`,
                                    description: `${BooruService.providers[provider.target].description}`,
                                }
                            })
                            searchTimer.stop();
                            return
            ...
            RowLayout { // Controls
                ...
                ApiInputBoxIndicator { // Tool indicator
                    icon: "api"
                    text: BooruService.providers[BooruService.currentProvider].name
                    tooltipText: TranslationService.tr("Current API endpoint: %1\nSet it with %2mode PROVIDER")
                        .arg(BooruService.providers[BooruService.currentProvider].url)
                ...
                MouseArea { // NSFW toggle
                    ...
                    RowLayout {
                        ...
                        StyledText {
                            ...
                            text: TranslationService.tr("Allow NSFW")
                        }
                        StyledSwitch {
                            id: nsfwSwitch
                            enabled: BooruService.currentProvider !== "zerochan"
                            scale: 0.6
                            Layout.alignment: Qt.AlignVCenter
                            checked: (Persistent.states.booru.allowNsfw && BooruService.currentProvider !== "zerochan")
                            onCheckedChanged: {
