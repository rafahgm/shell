...
Singleton {
    ...
    function friendlyDeviceName(node) {
        return node ? (node.nickname || node.description || TranslationService.tr("Unknown")) : TranslationService.tr("Unknown");
    }
    function appNodeDisplayName(node) {
        if (!node) return TranslationService.tr("Unknown");
        return (node.properties?.["application.name"] || node.description || node.name || TranslationService.tr("Unknown"))
    }
    ...
    Connections { // Protection against sudden volume changes
        ...
        function onVolumeChanged() {
            ...
            if (newVolume - lastVolume > maxAllowedIncrease) {
                ...
                root.sinkProtectionTriggered(TranslationService.tr("Illegal increment"));
            } else if (newVolume > maxAllowed || newVolume > root.hardMaxValue) {
                root.sinkProtectionTriggered(TranslationService.tr("Exceeded max allowed"));
