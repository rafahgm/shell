...
Singleton {
        ...
        },
        {
            ...
            execute: args => {
                if (!/^(\d+)/.test(args.trim())) {
                    ...
                    Quickshell.execDetached(["notify-send", TranslationService.tr("Superpaste"), TranslationService.tr("Usage: <tt>%1superpaste NUM_OF_ENTRIES[i]</tt>\nSupply <tt>i</tt> when you want images\nExamples:\n<tt>%1superpaste 4i</tt> for the last 4 images\n<tt>%1superpaste 7</tt> for the last 7 entries").arg(Config.options.search.prefix.action), "-a", "Shell"]);
                    return;
                }
    ...
    property bool clipboardWorkSafetyActive: {
        ...
        const sensitiveNetwork = (StringUtils.stringListContainsSubstring(NetworkService.networkName.toLowerCase(), Config.options.workSafety.triggerCondition.networkNameKeywords));
    ...
    property list<var> results: {
        ...
        if (root.query.startsWith(Config.options.search.prefix.clipboard)) {
            ...
            return Cliphist.fuzzyQuery(searchString).map((entry, index, array) => {
                ...
                return resultComp.createObject(null, {
                    ...
                    actions: [resultComp.createObject(null, {
                            name: TranslationService.tr("Copy"),
                            ...
                            name: TranslationService.tr("Delete"),
                            iconName: "delete",
                            iconType: LauncherSearchResult.IconType.Material,
                            execute: () => {
            ...
            return Emojis.fuzzyQuery(searchString).map(entry => {
                ...
                return resultComp.createObject(null, {
                    ...
                    verb: TranslationService.tr("Copy"),
                    type: TranslationService.tr("Emoji"),
                    execute: () => {
        ...
        const mathResultObject = resultComp.createObject(null, {
            ...
            verb: TranslationService.tr("Copy"),
            type: TranslationService.tr("Math result"),
        ...
        const appResultObjects = AppSearchService.fuzzyQuery(StringUtils.cleanPrefix(root.query, Config.options.search.prefix.app)).map(entry => {
            return resultComp.createObject(null, {
                type: TranslationService.tr("App"),
                id: entry.id,
                name: entry.name,
                iconName: entry.icon,
                iconType: LauncherSearchResult.IconType.System,
                verb: TranslationService.tr("Open"),
                execute: () => {
        ...
        const commandResultObject = resultComp.createObject(null, {
            ...
            verb: TranslationService.tr("Run"),
            type: TranslationService.tr("Command"),
            fontType: LauncherSearchResult.FontType.Monospace,
            iconName: 'terminal',
            iconType: LauncherSearchResult.IconType.Material,
            execute: () => {
        ...
        const webSearchResultObject = resultComp.createObject(null, {
            ...
            verb: TranslationService.tr("Search"),
            type: TranslationService.tr("Web search"),
        ...
        const launcherActionObjects = root.allActions.map(action => {
            ...
            if (actionString.startsWith(root.query) || root.query.startsWith(actionString)) {
                return resultComp.createObject(null, {
                    ...
                    verb: TranslationService.tr("Run"),
                    type: TranslationService.tr("Action"),
