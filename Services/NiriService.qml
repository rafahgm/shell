...
Singleton {
    ...
    Process {
        ...
        stderr: StdioCollector {
            onStreamFinished: {
                ...
                NotificationService.sendErrorNotification("niri: failed to load config", configValidationOutput);
            }
        }
    ...
    function handleScreenshotCaptured(data) {
        ...
        if (pendingScreenshotPath && data.path === pendingScreenshotPath) {
            const editor = Quickshell.env("SCREENSHOT_EDITOR");
            let command;
            if (editor === "satty") {
    ...
    function screenshot() {
        ...
        const path = `${Directories.screenshotsPath}/screenshot-${timestamp}.png`;
    ...
    function doGenerateNiriLayoutConfig() {
        ...
        const radius = Config.options?.appearance?.borderRadius ?? 12;
        const gaps = Config.options?.appearance?.gaps ?? 4;
        const borderSize = Config.options?.appearance?.borderSize ?? 4;

        const warning = `// ! DO NOT EDIT !
// ! AUTO-GENERATED !
...
`;
        ...
        const configContent = warning + `layout {
    gaps ${gaps}
...
window-rule {
    ...
    draw-border-with-background false
        ...
        const alttabContent = warning + `recent-windows {
    highlight {
        corner-radius ${radius}
    }
}`;

        const configPath = Directories.niriConfig + "/layout.kdl";
        const alttabPath = Directories.niriConfig + "/alttab.kdl";

        writeConfigProcess.configContent = configContent;
        writeConfigProcess.configPath = configPath;
        writeConfigProcess.command = ["sh", "-c", `cat > "${configPath}" << 'EOF'\n${configContent}\nEOF`];
        writeConfigProcess.running = true;

        writeAlttabProcess.alttabContent = alttabContent;
        writeAlttabProcess.alttabPath = alttabPath;
        writeAlttabProcess.command = ["sh", "-c", `cat > "${alttabPath}" << 'EOF'\n${alttabContent}\nEOF`];
        writeAlttabProcess.running = true;

        for (const name of ["outputs", "binds", "cursor", "windowrules", "colors", "alttab", "layout"]) {
            const path = `${Directories.niriConfig}/${name}.kdl`;
            Proc.runCommand("niri-ensure-" + name, ["sh", "-c", `[ ! -f "${path}" ] && touch "${path}" || true`], (output, exitCode) => {
                if (exitCode !== 0)
    ...
    function generateNiriBlurrule() {
        ...
        const blurrulePath = Directories.niriConfig + "/wpblur.kdl";
        const sourceBlurrulePath = FileUtils.trimFileProtocol(Qt.resolvedUrl("niri-wpblur.kdl"));

        writeBlurruleProcess.blurrulePath = blurrulePath;
        writeBlurruleProcess.command = ["sh", "-c", `cp --no-preserve=mode "${sourceBlurrulePath}" "${blurrulePath}"`];
    ...
    function generateNiriCursorConfig() {
        ...
        const cursorPath = Directories.niriConfig + "/cursor.kdl";

        const settings = Config.options?.cursor ?? null;

        if (!settings) {
            writeCursorProcess.cursorContent = "";
            writeCursorProcess.cursorPath = cursorPath;
            writeCursorProcess.command = ["sh", "-c", `: > "${cursorPath}"`];
            writeCursorProcess.running = true;
            return;
        }

        const isDefaultConfig = settings.theme === "System Default" && settings.size === 24 && !settings.hideWhenTyping && settings.hideAfterInactiveMs === 0;
...
`;
        ...
        let cursorContent = warning + `cursor {\n`;

        if (settings.theme && settings.theme !== "System Default")
            cursorContent += `    xcursor-theme "${settings.theme}"\n`;

        cursorContent += `    xcursor-size ${settings.size}\n`;

        if (settings.hideWhenTyping)
            cursorContent += `    hide-when-typing\n`;

        if (settings.hideAfterInactiveMs > 0)
            cursorContent += `    hide-after-inactive-ms ${settings.hideAfterInactiveMs}\n`;
        ...
        writeCursorProcess.command = ["sh", "-c", `printf '%s' '${escapedCursorContent}' > "${cursorPath}"`];
        writeCursorProcess.running = true;
    }
    ...
    function getOutputIdentifier(output, outputName) {
        if (Config.options?.displayNameMode === "model" && output.make && output.model) {
    ...
    function generateOutputsConfig(outputsData) {
        ...
        if (!data || Object.keys(data).length === 0)
            return;
        let kdlContent = `// Auto-generated - do not edit manually\n\n`;
        ...
        for (const outputName of sortedNames) {
            ...
            const niriSettings = Config.getNiriOutputSettings(identifier);
        ...
        const outputsPath = Directories.niriConfig + "/outputs.kdl";

        Proc.runCommand("niri-write-outputs", ["sh", "-c", `cat > "${outputsPath}" << 'EOF'\n${kdlContent}EOF`], (output, exitCode) => {
            if (exitCode !== 0) {
