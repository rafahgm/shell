...
Singleton {
    ...
    function refineData(data) {
        ...
        temp.lastRefresh = DateTimeService.time + " • " + DateTimeService.date;
        root.data = temp;
    }
    ...
    PositionSource {
        ...
        onValidityChanged: {
            if (!positionSource.valid) {
                ...
                Quickshell.execDetached(["notify-send", TranslationService.tr("Weather Service"), TranslationService.tr("Cannot find a GPS service. Using the fallback method instead."), "-a", "Shell"]);
